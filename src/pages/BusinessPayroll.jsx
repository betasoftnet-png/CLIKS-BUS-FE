...
const BusinessPayroll = () => {
    ...
    const handleProcessPayroll = (e) => {
        ...
        const createdPAY = {
            payroll_id: `PAY-2026-${Date.now().toString().slice(-3)}`,
            ...
            payslip_number: `PSN-2026-${Date.now().toString().slice(-3)}`,
            pan_number: 'XYZAB9911C',
            uan_number: '100223344999',
            esi_number: '3122334455009'
        };

        setPayrollRecords([createdPAY, ...payrollRecords]);
    ...
    return (
        <div style={{ padding: '2.5rem', background: '#F0F9F4', minHeight: '100vh', fontFamily: "'Inter', sans-serif" }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '3rem' }}>
                <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem' }}>
                        ...
                        <h1 style={{ fontSize: '2rem', fontWeight: '850', color: '#064E3B', letterSpacing: '-0.02em' }}>Workforce Payroll Engine</h1>
                    </div>
                    <p style={{ color: '#475569', fontSize: '1.05rem', fontWeight: '500' }}>Manage monthly earnings structures, HRA allowances, EPF/ESI statutory compliance filings, TDS calculations, loans EMIs, and auto payouts.</p>
                </div>
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                    ...
                    >
                        <Sliders size={16} /> Allocate Employee Loan
                    </button>
                    <button 
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.5rem', marginBottom: '3rem' }}>
                {[
                    { label: 'Net Monthly Salary Expense', value: `₹${totalSalaryExpense.toLocaleString()}`, icon: CreditCard, color: '#1B6B3A', bg: '#F0FDF4' },
                    { label: 'Pending Compliance PF / ESI', value: '₹4,250 Filed', icon: ShieldCheck, color: '#10B981', bg: '#ECFDF5' },
                    { label: 'Outstanding Loan Balance', value: `₹${totalLoanOutstanding.toLocaleString()}`, icon: Sliders, color: '#F59E0B', bg: '#FFFBEB' },
                    { label: 'Total Payslips Generated', value: `${payrollRecords.length} Payslips`, icon: FileText, color: '#2563EB', bg: '#EFF6FF' }
                ].map((stat, idx) => (
            ...
            <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
                {[
                    { id: 'run', label: 'Monthly Payroll Register', icon: FileText },
                    { id: 'structures', label: 'Salary Structures (CTC)', icon: CreditCard },
                    { id: 'compliance', label: 'Compliance (PF / ESI / PAN)', icon: ShieldCheck },
                    { id: 'loans', label: 'Loans & Salary Advances', icon: Sliders }
                ].map(tab => (
                    <button 
                        key={tab.id}
