...
const BusinessSegregation = () => {
    ...
    return (
        <div style={{ padding: '2.5rem', background: '#F0F9F4', minHeight: '100vh', fontFamily: "'Inter', sans-serif" }}>
            ...
            {/* Stats Grid - Matching Billing UI */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.5rem', marginBottom: '3rem' }}>
                {[
                    { label: 'Active Strategies', value: segregations.length, icon: Zap, color: '#1B6B3A', bg: '#F0FDF4' },
                    { label: 'Avg. Split Pct', value: '100%', icon: PieChart, color: '#0D9488', bg: '#F0FDFA' },
                    { label: 'Verified Rules', value: segregations.length, icon: ShieldCheck, color: '#064E3B', bg: '#ECFDF5' },
                    { label: 'Active Reserves', value: segregations.reduce((acc, s) => acc + (s.allocations?.length || 0), 0), icon: Target, color: '#1B6B3A', bg: '#F0FDF4' }
                ].map((stat, idx) => (
